...
def get_boxes(image, width_threshold, height_threshold, thickness=4, type="double"):
    ...
    if type == "double":
        ...
        with_contours_int = cv2.drawContours(
            image, contours, -1, (0, 0, 255), thickness
        )
        ...
        contours, hierarchy = cv2.findContours(
            inverted_contour, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    for c in contours:
        ...
        # For multilingual
        if (
            (cv2.contourArea(c)) > (width * height) / 100000
            and h < (height / 4)
            and (w < width / 2)
            and cv2.contourArea(c) > 35
        ):
            # for funsd
